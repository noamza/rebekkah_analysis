...
%            'LineWidth', 2, 'Color','r')
            ...
            [draw_orientation_line,smallest_degree, smallest_degree_wall] = orientation_analysis(Cell.autocorr, rate_mat, Cell.six_orientation_pts, draw_orientation_line, max_inds) ;
...
%%%%%%divided PF_radius by 4. if changing bin size, remember change this
%%%%%%accordingly!!!!
...
for cen = 1:max_inds_len-1
   for cen2= (cen+1):max_inds_len
    ...
    if peak_distance < 1.25*(S.PF_radius/2) 
     too_close (h,1) = cen;
     too_close(h,2) = cen2;
...
%%%%%%%%%% divided PF_radius by 2. change if changing bin size back to 3
%%%%%%%%%% from 6!!!!!!!!!!!!!!

S.PF_radius = (0.65* S.PF_radius)/2; 
