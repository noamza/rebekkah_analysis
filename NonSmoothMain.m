...
parms.dir_save_pictures='C:\Users\Dori\Desktop\nonsmooth PF 8 images';
...
parms.dir_save_data = 'C:\Users\Dori\Desktop\nonsmooth PF 8 results'; 
...
%            'LineWidth', 2, 'Color','r')
            ...
            fig_pair= plot_ellipse(rate_mat_after_auto_pearson, Cell.module.major, Cell.module.minor ,Cell.module.phi, Cell.six_orientation_pts, fig_pair);
 
         
            
            draw_orientation_line= subplot(n,m,6);
            [draw_orientation_line,smallest_degree] = orientation_analysis(rate_mat_after_auto_pearson, rate_mat, Cell.auto_max_inds, Cell.six_orientation_pts, draw_orientation_line) ;
...
for cen = 1:max_inds_len-1
   for cen2= (cen+1):max_inds_len
    ...
    if peak_distance < 1.25*(S.PF_radius) 
     too_close (h,1) = cen;
     too_close(h,2) = cen2;
...
for cen=1:max_inds_len;
     for fig_i =1:size_x
         for j =1:size_y 
             if Distance(fig_i, j, max_inds(cen,1), max_inds(cen,2)) < S.PF_radius        
