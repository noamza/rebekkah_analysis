...
parms.dir_load_data = 'N:\users\rebekkah\final data smoothed\data sets\87 cell set 45 to 60 pos_t'; 
%parms.dir_save_pictures='N:\users\rebekkah\final data smoothed\examine first and last 5 mins fanfactor smooth\images';
parms.dir_save_data = 'N:\users\rebekkah\final data smoothed\data sets\fanofactor vs duration\results'; 
...
for i =1:length(file_names)
 ...
 %    clear dat;
    ...
    pos_t_len= length(Cell.pos.t);
        ...
        one_ind= 1:15000;
        two_ind= 15001:30000;
        three_ind= 30001:45000;
  
    
    pos_t_1= Cell.pos.t(one_ind);   %first half of session
    pos_t_2= Cell.pos.t(two_ind); %second half of session
    pos_t_3= Cell.pos.t(three_ind);
    ...
    if ~isempty(Cell.pos.x2)
        ...
        pos_mean_x_1= pos_mean_x(one_ind); %x position for first half of session
        pos_mean_x_2= pos_mean_x(two_ind); %x position for second half of session
        pos_mean_x_3= pos_mean_x(three_ind);
        pos_mean_y_1= pos_mean_y(one_ind); %y position for first half of session
        pos_mean_y_2= pos_mean_y(two_ind); %y position for second half of session
        pos_mean_y_3= pos_mean_y(three_ind);
        ...
        ff1= var(S_1.sorted_means)/mean(S_1.sorted_means);
        ff2= var(S_2.sorted_means)/mean(S_2.sorted_means);
        ff3= var(S_3.sorted_means)/mean(S_3.sorted_means);
        
        fanofactors= [ff1 ff2 ff3];
        
        duration_time= [1 2 3];
        
        fanofactor_coefficient= corrcoef(fanofactors, duration_time);  
...
%title1 =strcat ('Results_',file_name);
    save (title1,'fanofactor_coefficient');
