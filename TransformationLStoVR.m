...
parms.dir_save_data= 'N:\users\rebekkah\barry jeffery pivot point\FULL DATA SET\transformation results\ls to vr\results';
parms.dir_save_images= 'N:\users\rebekkah\barry jeffery pivot point\FULL DATA SET\transformation results\ls to vr\images';
...
for i =1:length(file_names)
    ...
    if isfield(C, 'shapeSeq') & C.shapeSeq{1}== 'ls'     % if arena 1 is a large square
        ...
        index = strfind(C.shapeSeq, 'vr');  %find index of vertical arena
        index=index';
    ...
    % rectangle= A.rate_mats.arena1;
     % square= A.zone_mats.arena1;
        ...
        if size_rectangle(1) > size_square(1)
            ...
            rectangle(size_rectangle(1)-(diff_value-1):size_rectangle(1), :) = [];
