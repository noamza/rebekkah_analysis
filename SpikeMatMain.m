function SpikeMatMain
% Date: 24 of July 2014
dbstop if error

parms.dir_load_data = 'N:\users\rebekkah\final data smoothed\data sets\data set with HD without circular rooms_ 202 cells';
...
parms.dir_save_pictures='N:\users\rebekkah\final data smoothed\data sets\images number of spikes 202 cells';
...
parms.dir_save_data = 'N:\users\rebekkah\final data smoothed\data sets\results number of spikes 202 cells';
...
%rate_mat and autocorr
    ...
    %%%ADD ONLY FOR ARENA ADJUSTING MAIN. 
     cd(parms.dir_load_data2)
     dat1= load(file_name);
     Cell1= dat1.S;
     cd(parms.dir_load_data)
    %%%REMOVE ABOVE IF NOT ADJUSTING FOR ARENA ANGLE.
    ...
    if ~isempty(Cell.pos.x2)
      ...
      [rate_mat, spike_mat_smooth]=CreateRateMap(pos_mean_x,pos_mean_y,Cell.pos.t,spk_x,spk_y,Cell.spk.t,parms);
      
      rate_mat= spike_mat_smooth; % USE SPIKE MAT INSTEAD TO THEN COMPARE TO RATE MAT
...
count = count - 1;
    ...
    function S= get_zones(rate_mat, S)
        ...
        %% finds sum of spikes for each zone
        ...
        for cen = 1:number_zones_len;
              ...
              mean_values_list(mean_values_len) = sum(rate_mat(find_spk));
...
%             mean_values_list(isnan(mean_values_list)) = []; %remove NaNs
        ...
        for cen=1:number_zones_len;
            zone_mat(find(zone_mat==number_zones(cen))) = mean_values_list(cen);
        end
        ...
        peak_rates_list=mean_values_list;
