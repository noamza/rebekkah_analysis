...
gaussian_mat(centers(h,1), centers(h,2))= centervalue*40;
end

gaussian_mat_orig= gaussian_mat;
gaussian_mat= zeros(gsize + 10); % pad with edges
new_size= size(gaussian_mat);
gaussian_mat(6:new_size(1)-5, 6:new_size(2)-5)= gaussian_mat_orig; % gaussian mat with padded edges 
...
gaussian_mat= gaussian_mat(6:new_size(1)-5, 6:new_size(2)-5);
